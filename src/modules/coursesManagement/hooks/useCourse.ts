...
const useCourse = (periodId: string | undefined, courseId: string | undefined) => {
  ...
  const load = async () => {
    try {
      ...
      setLoading(true);
      const courseData = (await fetchPeriodCourseById(periodId, courseId)) as Course; // Fetch course data by ID
      if (courseData) {
